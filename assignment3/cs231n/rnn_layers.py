...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  cand_h = np.dot(prev_h, Wh) + np.dot(x, Wx) + b
  next_h = np.tanh(np.dot(prev_h, Wh) + np.dot(x, Wx) + b)
  cache = (prev_h, Wh, x, Wx, cand_h)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  prev_h, Wh, x, Wx, cand_h = cache
  # the derivative of tanh is 1 -tanh**
  dtanh = (1 - np.square(np.tanh(cand_h))) * dnext_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  #############################################################################
  N, H = prev_h.shape
  # Evaluate activation vectors (we have N of them)
  ...
  Ao = A[:,2*H:3*H]
  Ag = A[:,3*H:]
  # Compute the corresponding gates
  i = sigmoid(Ai)
  f = sigmoid(Af)
  o = sigmoid(Ao)
  g = np.tanh(Ag)
